-- Add Feed Action Support
-- Run this entire file in Supabase SQL Editor

-- Step 1: Add last_fed_at column to pet_status table
ALTER TABLE public.pet_status
ADD COLUMN IF NOT EXISTS last_fed_at timestamptz;

-- Step 2: Update record_pet_action function to include feed action
...
BEGIN
    ...
    ELSIF action_type = 'feed' THEN
        -- Feed action: 1 hour cooldown, no stat changes, just awards hearts
    ...
    ELSIF action_type = 'plant' THEN
        -- Plant watering: 15 minute cooldown, no stat changes, just awards hearts
